...
const longestName = "00: Converting the Maiden";
...
export class Hexagram {
  ...
  constructor(public top: Trigram, public bottom: Trigram) {
    ...
    if (this.name.length < longestName.length) {
      this.name = `${this.number}: ${this.name}`;
      const paddingRequired: number = longestName.length - this.name.length;
      const prePad = paddingRequired / 2;
      this.name = this.name.padStart(this.name.length + prePad, " ");
      const postPad = paddingRequired % 2 == 0 ? prePad : prePad + 1;
      this.name = this.name.padEnd(this.name.length + postPad, " ");
    }
  }
}